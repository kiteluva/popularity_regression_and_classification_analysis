---
title: "regression and classification outputs"
author: "Kiluva J, Wendy A, Daniellah M, Sururu H."
date: "`r Sys.Date()`"
output:
  pdf_document:
    latex_engine: xelatex  # THIS IS THE KEY CHANGE
---
...
if (ncol(numeric_features_for_density) > 0) {
  ...
  all_features_density_plot <- ggplot(data = data_long_features, aes(x = Value)) +
    geom_density(fill = "skyblue", alpha = 0.7, color = "steelblue") +
...
plot_roc_curves_gg <- function(model, test_data, model_name) {
  ...
  roc_plot <- ggplot(roc_data_df, aes(x = FPR, y = TPR, color = Class)) +
    ...
    theme_dark_custom() +
    scale_color_manual(values = c("very_low" = "green", "low" = "blue", "high" = "orange", "very_high" = "red")) +
    theme(legend.position = "bottom")
...
actual_vs_predicted_with_colored_zones_plot <- ggplot(plot_data_actual_vs_predicted_with_predicted_classes,
  ...
  geom_vline(xintercept = very_low_threshold, linetype = "dotted", color = "orange", size = 0.8) +
  geom_vline(xintercept = high_threshold, linetype = "dotted", color = "blue", size = 0.8) +
  geom_vline(xintercept = very_high_threshold, linetype = "dotted", color = "green", size = 0.8) +
  
  geom_hline(yintercept = very_low_threshold, linetype = "dotted", color = "orange", size = 0.8) +
  geom_hline(yintercept = high_threshold, linetype = "dotted", color = "blue", size = 0.8) +
  geom_hline(yintercept = very_high_threshold, linetype = "dotted", color = "green", size = 0.8) +
  ...
  scale_color_manual(values = c("very_low" = "green",
                                "low" = "blue",
                                "high" = "orange",
                                "very_high" = "red")) +
...
accuracy_plot <- ggplot(accuracy_comparison_df, aes(x = Model, y = Accuracy, fill = Model)) +
  ...
  labs(title = "Comparison of Average Model Accuracy",
       ...
       y = "Accuracy") +
  theme_dark_custom() +
  scale_fill_manual(values = c("Random Forest" = "green", "XGBoost" = "orange"))
